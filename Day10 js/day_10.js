const companies = new Set([1, 3, 3, 4, 4, 5, 5, 6]);
...
for (const language of setOfLang) {
  ...
  const count = [];
  const counts = {};
  const filterLang = languages.filter((lang) => lang === language);
  console.log(filterLang);
  count.push({ language: language, counts: filterLang.length });
  console.log(count);
}
...
//filter out number of time a language appear
...
const capital = new Map(count_ries);
console.log(capital);
...
const countriesMap = ["Finland", "Sweden", "Norway"];

for ([country] of countriesMap) {
  console.log(country, country.length);
}
const countries0 = ["Finland", "Sweden", "Norway"];
const countriez = new Map(
  countries0.map((country) => [country, country.length])
);
...
const intersect = Array.from(A1).filter((num) => B1.has(num));
console.log(intersect);

// Find a with b
const dif = Array.from(A1).filter((num) => !B1.has(num));
console.log(dif);
...
const CountryArray = countries_s
...
const arraySet = new Set(CountryArray);
console.log(arraySet);

// for (let con in countryCount) {
//   console.log(con, countryCount[con]);
// }

const mostSpokenLanguages = (CountryArray, count) => {
  let countryCount = CountryArray.map((language) => language.languages)
    .flat()
    .reduce((acc, cur) => {
  ...
  return (countryCount = Object.entries(countryCount))
    .sort((a, b) => b[1] - a[1])
    .map((con) => ({ language: con[0], count: con[1] }))
    .slice(0, count);
};
console.log(mostSpokenLanguages(countries_s, 10));
console.log(mostSpokenLanguages(countries_s, 10));
